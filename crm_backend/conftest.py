...
def user(db):
    ...
    return User.objects.create_user(
        username="testuser", email="test@example.com", password="testpass123", role=User.Role.RESIDENT
    )
...
def personal_account(db, apartment):
    ...
    return PersonalAccount.objects.create(apartment=apartment, account_number="ACC-001", balance=0)
...
def department(db):
    ...
    return Department.objects.create(name="Maintenance", description="Building maintenance department")
...
def employee(db, staff_user, department):
    ...
    return Employee.objects.create(
        user=staff_user, department=department, role=Employee.Role.MASTER, phone="+905551234569"
    )

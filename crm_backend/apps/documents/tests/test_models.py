...
class TestDocument:
    ...
    def test_document_str(self, user):
        doc = Document.objects.create(title="Contract 101", uploaded_by=user)
        assert str(doc) == "Contract 101"

    def test_ordering_by_created_at(self, user):
        doc1 = Document.objects.create(title="First", uploaded_by=user)
        doc2 = Document.objects.create(title="Second", uploaded_by=user)
        docs = list(Document.objects.all())
        assert docs[0] == doc2
        assert docs[1] == doc1

    def test_uploaded_by_nullable(self):
        doc = Document.objects.create(title="Anonymous Upload")
    ...
    def test_related_name_user(self, user):
        doc = Document.objects.create(title="User Upload", uploaded_by=user)
